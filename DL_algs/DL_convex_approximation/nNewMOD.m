function [D,Gamma,err,gerr,ratio] = NewMOD(params,varargin)
%  Method of Optimal Directions (MOD) dictionary learning with the convex 
%  approximation approach proposed in the following paper:
...
%  NewMOD dictionary training.
%  [D,GAMMA] = NewMOD(PARAMS) runs the NewMOD dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  NewMOD has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = NewMOD(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = NewMOD(PARAMS) computes the target function values on
...
%
%  [...] = NewMOD(...,VERBOSE) where VERBOSE is a character string, specifies
...
%
%  [...] = NewMOD(...,MSGDELTA) specifies additional messages to be printed
...
%  causes NewMOD to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of NewMOD iterations to perform. If not
...
for iter = 1:iternum
  ...
  lam=lam0*(lamf/lam0)^(iter/iternum);
  % Update the whole dictionary
  GammaE=Gammao-Gamma;
  E=data+Do*GammaE;
%   Gammao=Gamma;
  s1=E*Gammao'+lam*D;
  s2=(Gammao*Gammao')+lam*eye(dictsize);
  D=s1/s2;
  D = normcols(D);
  Do=D;
  Gammao=Gamma;
