function [D,Gamma,err,gerr,ratio] = MDU_DL(params,varargin)
%  Multiple Dictionary Update (MDU) dictionary learning proposed in the 
%  following paper:

% L. N. Smith and M. Elad, “Improving dictionary learning: Multiple
% dictionary updates and coefficient reuse,” IEEE Signal Process. Lett.,
% vol. 20, no. 1, pp. 79–82, 2013.


%  [D,GAMMA] = MDU_DL(PARAMS) runs the MDU_DL dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  MDU_DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = MDU_DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = MDU_DL(PARAMS) computes the target function values on
...
%
%  [...] = MDU_DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%
%  [...] = MDU_DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes MDU_DL to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of MDU_DL iterations to perform. If not
...
for iter = 1:iternum
  ...
  s1=(data*Gamma');
  s2=(Gamma*Gamma')+ 1e-4*speye(dictsize);
  D=s1/s2;
  D = normcols(D);
