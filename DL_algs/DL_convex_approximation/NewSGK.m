function [D,Gamma,err,gerr,ratio] = NewSGK(params,varargin)

%  Sequential Generalization of K-means (SGK) idea with the convex approximation 
...
%  NewSGK dictionary training.
%  [D,GAMMA] = NewSGK(PARAMS) runs the NewSGK dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  NewSGK has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = NewSGK(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = NewSGK(PARAMS) computes the target function values on
...
%
%  [...] = NewSGK(...,VERBOSE) where VERBOSE is a character string, specifies
...
%
%  [...] = NewSGK(...,MSGDELTA) specifies additional messages to be printed
...
%  causes NewSGK to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of NewSGK iterations to perform. If not
