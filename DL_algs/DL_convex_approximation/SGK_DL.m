function [D,Gamma,err,gerr,ratio] = SGK_DL(params,varargin)
%  Sequential Generalization of K-means dictionary learning (SGK_DL) proposed
%  by the following paper:

%  S. K. Sahoo and A. Makur, “Dictionary training for sparse representation
%  as generalization of K-Means clustering,” IEEE Signal Process. Lett., 
%  vol. 20, no. 6, pp. 587–590, 2013.


%  [D,GAMMA] = SGK_DL(PARAMS) runs the SGK_DL dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  SGK_DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = SGK_DL(PARAMS) also returns the target function values
...
%
%  [D,GAMMA,ERR,GERR] = SGK_DL(PARAMS) computes the target function values on
...
%
%  [...] = SGK_DL(...,VERBOSE) where VERBOSE is a character string, specifies
...
%
%  [...] = SGK_DL(...,MSGDELTA) specifies additional messages to be printed
...
%  causes SGK_DL to run in silent mode, ignoring the value of MSGDELTA.
%
%
...
%      Specifies the number of SGK_DL iterations to perform. If not
