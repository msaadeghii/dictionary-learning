function [D,Gamma,err,gerr,ratio]=APrU_DL(params,varargin)
%   APrU_DL: Atom-Profile Updating Dictionary Learning algorithm introduced in:
%
...
%
%  [D,GAMMA] = APrU_DL(PARAMS) runs the APrU_DL dictionary training algorithm on
...
%      D,GAMMA
%
%  [D,GAMMA,ERR] = APrU_DL(PARAMS) also returns the target function values
...
%      Specifies the number of APrU_DL iterations to perform. If not
...
for iter = 1:iternum
    ...
    Gamma = GrFISTA(data,D,Gamma,2*lam,15,map);
    ...
    %%%%%  compute representation error for test data  %%%%%
    ...
    % compute dictionary recovry rate
    if ctr
