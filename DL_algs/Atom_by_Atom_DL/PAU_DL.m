function [D,Gamma,err,gerr,ratio]=PAU_DL(params,varargin)
%   PAU_DL: Parallel Atom-Update Dictionary Learning introduced in:
%
...
%
%
%  [D,GAMMA] = PAU_DL(PARAMS) runs the PAU_DL dictionary training algorithm on
%  the specified set of signals, returning the trained dictionary D and the
%  signal representation matrix GAMMA.
%
%  PAU_DL has two modes of operation: sparsity-based and error-based. For
...
%
%  [D,GAMMA,ERR] = PAU_DL(PARAMS) also returns the target function values
...
%      Specifies the number of PAU_DL iterations to perform. If not
...
for iter = 1:iternum
   ...
   % Atom-by-atom dictionary update
    for k=1:inniter
        for j = 1:dictsize
            [D(:,j),gamma_j,data_indices,unused_sigs,replaced_atoms] = optimize_atom(data,D,j,Gamma,unused_sigs,replaced_atoms);
            Gamma(j,data_indices) = gamma_j;
    ...
    % compute percentage of recovered atoms
    if ctr
